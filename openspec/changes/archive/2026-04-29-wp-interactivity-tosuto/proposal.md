...
- New WordPress plugin (`wp-tosuto`) providing a toast notification component via the Interactivity API
...
- PHP API (`wp_tosuto()`) and JavaScript API via the Interactivity API store for programmatic toast creation
...
- `tosuto-store`: Reactive state management for toast notifications using the Interactivity API store — handles adding, removing, stacking order, and auto-dismiss timers
- `tosuto-renderer`: Visual rendering of the toast container and individual toast items — animations, dismiss button, variant styling, and content projection
- `tosuto-api`: Public PHP and JS APIs for creating toasts — server-side `wp_tosuto()` helper that enqueues toasts on page load, and client-side store actions for runtime toast creation
