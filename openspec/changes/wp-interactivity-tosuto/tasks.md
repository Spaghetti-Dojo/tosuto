...
- [x] 3.1 Create `sources/server/Toaster/render.php` — render the toast container `<div>` with `data-wp-interactive="wp-tosuto"`, `data-wp-init="callbacks.init"`, `role="status"`, `aria-live="polite"`, fixed positioning, and `data-wp-each="state.toastList"` for toast items (using the computed getter that converts the Map to an array)
- [x] 3.2 Add single-instance guard in the render file — use static flag to prevent duplicate containers
- [x] 3.3 Add toast item template inside `<template data-wp-each="state.toastList">` — write `content` via `data-wp-watch="callbacks.renderContent"` (sets `innerHTML` on the content element), bind the variant via `data-wp-bind--data-variant="context.item.variant"` (styled via CSS `[data-variant]` attribute selectors), hide the dismiss button via `data-wp-bind--hidden="!context.item.dismissable"`, and attach hover event directives (`callbacks.pauseToast` / `callbacks.resumeToast`)
...
- [x] 5.3 In the `Api` module, hook into `wp_body_open` at a late priority to render: if the queue is non-empty, call `wp_interactivity_state('wp-tosuto', ['toasts' => $queue])` and render the toaster block via `do_blocks('<!-- wp:wp-tosuto/toaster /-->')`, storing the output in a static buffer. Hook into `wp_footer` at a late priority to print the buffered output. This two-phase approach ensures block rendering triggers script/style enqueueing early enough. No defaulting or id assignment happens here; the queue is already final
- [x] 5.4 In the same `wp_body_open` hook, when the queue is empty, apply the `wp-tosuto.api.render-empty` filter (default `true`) and resolve its value: `true` → emit the toaster block; `false` → emit nothing; `is_callable(...)` → `call_user_func()` it with no arguments and use the coerced-to-bool result (so `'is_singular'`, `'is_single'`, `'is_post_type_archive'`, or a closure can all be passed); any other value → coerce to bool. When the resolved decision is "do not emit", neither `wp_interactivity_state()` nor `do_blocks()` runs, so no `wp-tosuto` view script module or stylesheet is enqueued
